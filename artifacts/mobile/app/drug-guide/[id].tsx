  ...
  return (
    ...
    >
      <View style={infoStyles.cardHeader}>
        ...
        <Text style={[infoStyles.cardTitle, { color: highlight ? colors.primary : colors.foreground }]}>
          {title}
        </Text>
      </View>
      <Text style={[infoStyles.cardBody, { color: colors.foreground }]}>{content}</Text>
    </View>
  );
});
...
const ListCard = memo(function ListCard({
  ...
  return (
    <View style={[infoStyles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      {items.map((item, i) => (
        <View key={i} style={[infoStyles.listRow, i < items.length - 1 && infoStyles.listRowGap]}>
          <View style={[infoStyles.bullet, { backgroundColor: color }]} />
          <Text style={[infoStyles.listItem, { color: colors.foreground }]}>{item}</Text>
        </View>
      ))}
    </View>
  );
...
const PREGNANCY_DESC: Record<string, string> = {
...
export default function DrugDetailScreen() {
  ...
  const drug = useMemo(() => getDrugById(id ?? ""), [id]);
  ...
  }, [drug, bookmarked, addBookmark, removeBookmark]);
  ...
  const pregnancyColor = drug.pregnancyCategory === "X" ? "#FEE2E2" : "#F0FDF4";
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.metaRow}>
        ...
        <View style={[styles.badge, { backgroundColor: pregnancyColor }]}>
          <Text style={[styles.badgeText, { color: pregnancyTextColor }]}>
      ...
      >
        ...
        {activeTab === 2 && (
          <>
            ...
            <InfoCard  title="Pregnancy Safety" icon="heart"
              content={`Category ${drug.pregnancyCategory}: ${PREGNANCY_DESC[drug.pregnancyCategory] ?? "Category not specified."}`}
              colors={colors}
            />
          </>
        )}
